...
class DeviceCamera
{
    ...
    private const int ccdWidth = 2464; // Constants to define the ccd pixel dimenstions
    ...
    public string SensorName
    {
        get
        {
            tl.LogMessage("SensorName Get", "AVT Manta G-507");
            return "AVT Manta G-507";
        }
    }
    ...
    // Change the return type from 'ushort*' to 'ushort[]' to avoid using pointers.
    // Remove the pointer usage in the method signature and implementation.

    public ushort[] StartExposure(double Duration, bool Light)
    {
        ...
        ushort[] image_data = new ushort[ccdWidth * ccdHeight];
        var vmb = IVmbSystem.Startup(); // API startup (loads transport layers)
        var camera = vmb.GetCameras()[0]; // Get the first available camera    
        var openCamera = camera.Open(); // open the camera
        var features = openCamera.Features;
        // Set camera attributes
        features.ExposureTimeAbs = (int)(Duration * 1e6); // Set the exposure time value in us
        features.Gain = 0; // Set the gain value in dB
        openCamera.FrameReceived += (s, e) =>
        {
            ...
            Marshal.Copy(imagePtr, imageBufferData, 0, (int)pixelCount * sizeof(ushort));
        ...
        var acquisition = openCamera.StartFrameAcquisition();
        Thread.Sleep(100);
